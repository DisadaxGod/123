...
public class Table extends AbstractTableModel {
    ...
    public void setValueAt( int rowIndex) {
        ...
        if(p instanceof Book){
            p=new Book(p.getAuthorName(),p.getName(),p.getPages());
        }
        else if(p instanceof Magazine){
            p=new Magazine(p.getAuthorName(),p.getName(),((Magazine) p).getGenre(),p.getPages());
        }
        else if(p instanceof TextBook){
            p=new TextBook(p.getAuthorName(),p.getName(),((TextBook) p).getSubject(),p.getPages());
        }
        data.set(rowIndex,p);
        this.fireTableDataChanged();
    }
